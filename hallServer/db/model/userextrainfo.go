...
//存在就更新， 不存在就插入
func (op *userextrainfoOp) InsertUpdate(obj *Userextrainfo, m map[string]interface{}) error {
	sql := "insert into userextrainfo(UserId,MbPayTotal,MbVipLevel,PayMbVipUpgrade,MbTicket) values(?,?,?,?,?) ON DUPLICATE KEY UPDATE "
