...
type UserSpread struct {
	...
	SpreadUid int64 `db:"spread_uid" json:"spread_uid"` // 被我领取的推广人id
...
//存在就更新， 不存在就插入
func (op *userSpreadOp) InsertUpdate(obj *UserSpread, m map[string]interface{}) error {
	sql := "insert into user_spread(user_id,spread_uid,status) values(?,?,?) ON DUPLICATE KEY UPDATE "
...
// 用主键做条件，更新map里包含的字段名
func (op *userSpreadOp) UpdateWithMap(user_id int64, spread_uid int64, m map[string]interface{}) error {
	return op.UpdateWithMapTx(db.DB, user_id, spread_uid, m)
}

// 用主键做条件，更新map里包含的字段名
func (op *userSpreadOp) UpdateWithMapTx(ext sqlx.Ext, user_id int64, spread_uid int64, m map[string]interface{}) error {
