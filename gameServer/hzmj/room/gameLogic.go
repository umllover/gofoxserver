...
type GameLogic struct {
	CardDataArray []uint8	//扑克数据
	MagicIndex uint8			//钻牌索引
}
...
func (lg *GameLogic)SetMagicIndex( cbMagicIndex uint8) {
	lg.MagicIndex = cbMagicIndex
}

//混乱扑克
func (lg *GameLogic) RandCardList(cbCardBuffer, OriDataArray []uint8){
	//混乱准备
	cbBufferCount := uint8( len(cbCardBuffer))
	cbCardDataTemp :=  make([]uint8,cbBufferCount)
	util.DeepCopy(&cbCardDataTemp, &OriDataArray)

	//混乱扑克
	var cbRandCount uint8
	var cbPosition uint8
	for ;;{
		if cbRandCount >= cbBufferCount {
			break
		}
		cbPosition=uint8( util.RandInterval(0, int(cbBufferCount-cbRandCount)))
...
//删除扑克
func (lg *GameLogic)RemoveCardByArray(cbCardData []uint8, cbCardCount uint8, cbRemoveCard []uint8,  cbRemoveCount uint8) bool{
	//效验扑克

	//定义变量
	var cbDeleteCount uint8
	var cbTempCardData []uint8
	...
	//置零扑克
	for i := uint8(0); i<cbRemoveCount; i++{
		for j := uint8(0); j<cbCardCount; j++{
...
//删除扑克
func (lg *GameLogic)RemoveCardByCnt (cbCardIndex, cbRemoveCard []uint8,  cbRemoveCount uint8) bool{
	//删除扑克
	for  i := uint8(0); i<cbRemoveCount; i++{
		//效验扑克
...
//删除扑克
 func (lg *GameLogic)RemoveCard(cbCardIndex []uint8, cbRemoveCard uint8) bool{
	//删除扑克
...
//财神判断
 func (lg *GameLogic)IsMagicCard(cbCardData uint8) bool{
...
//花牌判断
func (lg *GameLogic)IsHuaCard(cbCardData uint8) bool {

	return cbCardData >= 0x38;
}

//花牌判断
func (lg *GameLogic)HuaCardCnt(cbCardIndex []uint8) uint8{
	cbHuaCardCount := uint8(0)
...
//排序,根据牌值排序
func (lg *GameLogic)SortCardList(cbCardData []uint8, cbCardCount uint8) bool{
	//数目过虑
...
//动作等级
func (lg *GameLogic)GetUserActionRank(cbUserAction uint8) uint8{
	//胡牌等级
	if (cbUserAction&WIK_CHI_HU != 0) { return 4 }

	//杠牌等级
	if (cbUserAction&WIK_GANG != 0) { return 3 }

	//碰牌等级
	if (cbUserAction&WIK_PENG != 0) { return 2 }

	//上牌等级
	if (cbUserAction&(WIK_RIGHT|WIK_CENTER|WIK_LEFT) != 0) { return 1 }
...
//自动出牌
func (lg *GameLogic)AutomatismOutCard(cbCardIndex,  cbEnjoinOutCard []uint8,  cbEnjoinOutCardCount uint8) uint8 {
	// 先打财神
	if(lg.MagicIndex != MAX_INDEX){
	...
	var cbCardData uint8
	var cbOutCardIndex = uint8(MAX_INDEX)
	var cbOutCardIndexCount uint8
	for i := uint8(MAX_INDEX - 7); i < MAX_INDEX - 1; i++{
	...
	if(cbOutCardIndex != MAX_INDEX){
		...
		bEnjoinCard := false;
		for k := uint8(0); k < cbEnjoinOutCardCount; k++{
	...
	for i := 0; i < 5; i++{
		for j := 0; j < 3; j++{
			...
			if(cbCardIndex[j * 9 + i] > 0){
				cbOutCardIndex = uint8(j * 9 + i);
			}else if(cbCardIndex[j * 9 + (9 - i - 1)] > 0){
				cbOutCardIndex = uint8(j * 9 + (9 - i - 1))
			}
...
//吃牌判断
func (lg *GameLogic)EstimateEatCard (cbCardIndex []uint8,  cbCurrentCard uint8) uint8{
	//参数效验
	cbCurrentIndex := lg.SwitchToCardIndex(cbCurrentCard)

	//过滤判断
	if cbCurrentIndex == lg.MagicIndex { return WIK_NULL}
	if cbCurrentIndex == INDEX_REPLACE_CARD && lg.MagicIndex>=27 {return WIK_NULL}
	if cbCurrentCard >= 0x31 && cbCurrentIndex != INDEX_REPLACE_CARD {  return WIK_NULL}

	//变量定义
	var cbExcursion = []uint8{0,1,2}
	var cbItemKind = []uint8{WIK_LEFT,WIK_CENTER,WIK_RIGHT}

	//拆分分析
	var cbMagicCardIndex []uint8
	util.DeepCopy(cbMagicCardIndex, cbCardIndex)

	//如果有财神
	cbMagicCardCount := uint8(0)
	if(lg.MagicIndex != MAX_INDEX){
...
//碰牌判断
func (lg *GameLogic)EstimatePengCard( cbCardIndex []uint8,  cbCurrentCard uint8) uint8{
	//参数效验
...
//杠牌判断
func (lg *GameLogic)EstimateGangCard(cbCardIndex []uint8, cbCurrentCard uint8) uint8{
	//参数效验
...
//杠牌分析
func (lg *GameLogic)AnalyseGangCard(cbCardIndex []uint8, WeaveItem []*msg.WeaveItem,  cbWeaveCount uint8,  gangCardResult *TagGangCardResult)uint8{
	//设置变量
	cbActionMask := uint8(WIK_NULL)


	//手上杠牌
	for  i := uint8(0); i<MAX_INDEX; i++{
		if(i == lg.MagicIndex) {
			continue
		}
		if (cbCardIndex[i]==4){
	...
	//组合杠牌
	for  i := uint8(0); i<cbWeaveCount; i++{
...
func (lg *GameLogic)AnalyseGangCardEx(cbCardIndex []uint8, WeaveItem []*msg.WeaveItem,  cbWeaveCount, cbProvideCard uint8, gangCardResult *TagGangCardResult)uint8{
	//设置变量
	cbActionMask := uint8(WIK_NULL)
...
func (lg *GameLogic) GetCardColor( cbCardData uint8) uint8{ return cbCardData&MASK_COLOR;}
func (lg *GameLogic) GetCardValue( cbCardData uint8) uint8{ return cbCardData&MASK_VALUE;}

//吃胡分析
func (lg *GameLogic)AnalyseChiHuCard(cbCardIndex []uint8, WeaveItem []*msg.WeaveItem, cbWeaveCount, cbCurrentCard uint8, ChiHuRight int, b4HZHu bool) uint8{
	//变量定义
	cbChiHuKind :=  uint8(WIK_NULL)
	TagAnalyseItemArray := make([]*TagAnalyseItem, MAX_INDEX) //todo MAX_INDEX CTagAnalyseItemArray

	//构造扑克
	cbCardIndexTemp := make([]uint8, MAX_INDEX)
	...
	if (len(TagAnalyseItemArray) >0 ) {
		//牌型分析
// 		for  i := uint8(0); i< len(TagAnalyseItemArray); i++ {
// 			//变量定义
...
//听牌分析
func (lg *GameLogic)AnalyseTingCard2(cbCardIndex []uint8,   WeaveItem []*msg.WeaveItem,  cbWeaveCount uint8) uint8{
	//复制数据
	cbCardIndexTemp := make([]uint8, MAX_INDEX)
	util.DeepCopy(cbCardIndexTemp, cbCardIndex)
	...
	if((cbCardCount + 1) % 3 == 0){
		for i := 0; i < MAX_INDEX - MAX_HUA_INDEX; i++{
			...
			for j := uint8(0); j < MAX_INDEX-MAX_HUA_INDEX; j++{
...
//听牌分析
func (lg *GameLogic)AnalyseTingCard1(cbCardIndex []uint8,   WeaveItem []*msg.WeaveItem,  cbWeaveCount uint8, cbOutCard [][]uint8) uint8{
	//复制数据
	cbOutCount := uint8(0)
	cbCardIndexTemp := make([]uint8, MAX_INDEX)
	util.DeepCopy(cbCardIndexTemp, cbCardIndex)
	...
	if((cbCardCount-2)%3==0){
		for i := uint8(0); i < MAX_INDEX-MAX_HUA_INDEX; i++{
			if(cbCardIndexTemp[i] == 0)  {
				continue
				}
				cbCardIndexTemp[i]--;
...
func (lg *GameLogic)AnalyseTingCard(cbCardIndex []uint8,  WeaveItem []*msg.WeaveItem, cbWeaveCount uint8, cbOutCardData, cbHuCardCount []uint8, cbHuCardData[][]uint8)(uint8){
...
func (lg *GameLogic)GetHuCard( cbCardIndex []uint8,  WeaveItem []*msg.WeaveItem,  cbWeaveCount uint8, cbHuCardData []uint8) uint8{
	//复制数据
	cbCardIndexTemp := make([]uint8, MAX_INDEX)
	util.DeepCopy(cbCardIndexTemp,cbCardIndex)
...
//扑克转换
func (lg *GameLogic)SwitchToCardData(cbCardIndex uint8) uint8{
...
//扑克转换
func (lg *GameLogic)SwitchToCardIndex(cbCardData uint8) uint8 {
	return ((cbCardData & MASK_COLOR) >> 4) * 9 + (cbCardData & MASK_VALUE) - 1; //高位乘以9 + 低位 9 是系数 用来区分index的花色 index/ 9取整就是花色 所有的牌都是不会大于9 是余数
}

//扑克转换
func (lg *GameLogic)SwitchToCardData2( cbCardIndex, cbCardData []uint8) uint8{
	//转换扑克
	cbPosition := uint8(0)
	//财神
	if(lg.MagicIndex != MAX_INDEX){
		for i := uint8(0); i < cbCardIndex[lg.MagicIndex]; i++{
	...
	for  i := uint8(0); i<MAX_INDEX; i++{
		if(i == lg.MagicIndex && lg.MagicIndex != INDEX_REPLACE_CARD){
			...
			if(INDEX_REPLACE_CARD != MAX_INDEX){
				for j := uint8(0); j < cbCardIndex[INDEX_REPLACE_CARD]; j++{
		...
		if (cbCardIndex[i]!=0){
			for  j := uint8(0);j<cbCardIndex[i]; j++{ //牌展开
...
//扑克转换
func (lg *GameLogic)SwitchToCardIndex3(cbCardData []uint8, cbCardCount uint8, cbCardIndex []uint8) uint8{
	//设置变量
...
//有效判断
func (lg *GameLogic)IsValidCard( cbCardData uint8) bool{
	var cbValue = uint8(cbCardData & MASK_VALUE)
	var cbColor = uint8((cbCardData & MASK_COLOR) >> 4)
	return (((cbValue >= 1) && (cbValue <= 9) && (cbColor <= 2)) || ((cbValue >= 1) && (cbValue <= (7 + MAX_HUA_INDEX)) && (cbColor == 3)));
}

//扑克数目
func (lg *GameLogic)GetCardCount(cbCardIndex []uint8) uint8{
	//数目统计
	cbCardCount := uint8(0)
	for  i := uint8(0); i<MAX_INDEX; i++ {
...
//获取组合
func (lg *GameLogic)GetWeaveCard( cbWeaveKind, cbCenterCard uint8,  cbCardBuffer []uint8) uint8{
	//组合扑克
	switch cbWeaveKind {
	case WIK_LEFT:		//上牌操作
			//设置变量
...
func (lg *GameLogic)AddKindItem(TempKindItem *TagKindItem, KindItem []*TagKindItem,  cbKindItemCount uint8, bMagicThree bool) bool{ // todo BYTE &cbKindItemCount, bool &bMagicThree
	...
	if TempKindItem.MagicCount>=3{
		...
		cbNoMagicIndex := uint8(0)
		cbNoTempMagicIndex :=  uint8(0)
		for i := uint8(0); i < 3; i++{
		...
		bFind := false;
		for j := uint8(0); j < cbKindItemCount; j++ {
			for i := uint8(0); i < 3; i++ {
		...
		cbTempCardIndex := []uint8{0, 0}
		cbCardIndex :=  []uint8{0,0}
		cbCardCount  := uint8(0)
		for i := uint8(0); i < 3; i++{
			if(TempKindItem.ValidIndex[i] != lg.MagicIndex){
...
//分析扑克
func (lg *GameLogic)AnalyseCard(cbCardIndex []uint8,  WeaveItem []*msg.WeaveItem,  cbWeaveCount uint8,  TagAnalyseItemArray []*TagAnalyseItem) bool{ //todo , CTagAnalyseItemArray & TagAnalyseItemArray
	//计算数目
	...
	if (cbLessKindItem==0){
		...
		for  i := uint8(0); i<MAX_INDEX;  i++{
			if (cbCardIndex[i]==2 || (lg.MagicIndex != MAX_INDEX && i != lg.MagicIndex && cbCardIndex[lg.MagicIndex]+cbCardIndex[i]==2)){
				...
				//设置结果
				for j := uint8(0); j<cbWeaveCount; j++{
	...
	//拆分分析
	cbMagicCardIndex := make([]uint8, MAX_INDEX)
	util.DeepCopy(cbMagicCardIndex,cbCardIndex)


	//如果有财神
	cbMagicCardCount := uint8(0)
	cbTempMagicCount :=  uint8(0)
		...
		for i := uint8(0); i<MAX_INDEX-MAX_HUA_INDEX; i++{
			...
			if ((i<(MAX_INDEX-MAX_HUA_INDEX-9))&&((i%9)<7)){
				...
				if(cbMagicCardCount+cbMagicCardIndex[i]+cbMagicCardIndex[i+1]+cbMagicCardIndex[i+2] >= 3){
					var cbIndex = []uint8{ cbMagicCardIndex[i],cbMagicCardIndex[i+1],cbMagicCardIndex[i+2] }
					...
					cbValidIndex := make([]uint8, 3)
					for ; ;{
						if  nMagicCountTemp+cbIndex[0]+cbIndex[1]+cbIndex[2] < 3 {
							break
						}
						maxlen := uint8( len(cbIndex))
						for j := uint8(0); j <maxlen; j++{
							if(cbIndex[j] > 0) {
								cbIndex[j]--
	...
	if (cbKindItemCount>=cbLessKindItem){
		...
		//变量定义
		cbIndex := make([]uint8, MAX_WEAVE)
		for i := uint8(0); i < uint8(len(cbIndex)); i++{
			cbIndex[i] = i;
		}
		...
		for ;; {
			...
			cbTempMagicCount = uint8(0)
			for i := uint8(0); i<cbLessKindItem; i++ {
			...
			if(cbTempMagicCount <= cbMagicCardCount){
				...
				for i := uint8(0); i<cbLessKindItem*3; i++{
					//存在判断
...
func (lg *GameLogic)IsPengPeng(pTagAnalyseItem *TagAnalyseItem) bool{
	for i := uint8(0); i < uint8(len(pTagAnalyseItem.WeaveKind)); i++{
...
//是否麻七系列
func (lg *GameLogic)IsMaQi(cbCardIndex []uint8, cbWeaveCount uint8, ChiHuRight int64) bool{
	if(cbWeaveCount!=0) {
		return false;
	}

	cbGang := uint8(0)
	var cbMagicCount  uint8
	if lg.MagicIndex != MAX_INDEX {
...
//十三烂系列
func (lg *GameLogic)IsShiSanLan(cbCardIndex []uint8, cbWeaveCount uint8, ChiHuRight int64) bool{
	//组合判断
	if (cbWeaveCount!=0) {
		return false;
	}
