...
func SliceRand(source []interface{}, count int) []interface{} {
	sLen := len(source)
	temp := append([]interface{}{}, source...)

	var result []interface{}
	for ; sLen > 0; sLen-- {
		roll, _ := RandInt(0, sLen)
		result = append(result, temp[roll])
		temp = append(temp[0:roll], temp[roll+1:]...)
		if len(result) >= count {
			break
		}
	}
